...
def _hdfs_overwrite_enabled() -> bool:
    """Read HDFS_OVERWRITE fresh for each file upload."""
...
def hdfs_upload_tree(
    ...
    *,
    ...
    Behavior:
      * fail if src is not a directory or is empty;
      * create all remote directories first, sequentially;
      * upload files in parallel;
      * accumulate put failures and raise only after all futures finish;
      * do not roll back successfully uploaded files.
    """
    src = Path(src_path)
    if not src.is_dir():
